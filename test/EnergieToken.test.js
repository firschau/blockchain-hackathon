...
contract('EnergyToken', function (accounts) {
    before(async function () {
        ...
        marketAuthority = await IdentityContract.new('0x0000000000000000000000000000000000000000', 900, accounts[9], {
            from: accounts[9],
        })
        console.log(
            `Successfully deployed IdentityContract for Market Authority with address: ${marketAuthority.address}`
        )
        ...
        let physicalAssetAuthorityDeployment = await identityContractFactory.createIdentityContract({
            from: accounts[8],
        })
    ...
    it.skip('determines token IDs correctly.', async function () {
        // Get token ID.
    ...
    it.skip('converts between uints an enums.', async function () {
    ...
    it('can mint tokens.', async function () {
        ...
        console.log('idc 0 Address-- ' + idcs[0].options.address)
        console.log('receivedTokenId  ->  ' + receivedTokenId)
        // Pad token ID to full length.
        let receivedTokenIdPadded = receivedTokenId.toString('hex')
        console.log('reciveTokID in Hex  ->  ' + receivedTokenIdPadded)
        while (receivedTokenIdPadded.length < 64) {
            receivedTokenIdPadded = '0' + receivedTokenIdPadded
        }
        console.log('- - after Loop  - - ->  ' + receivedTokenIdPadded)
        let id = '0x' + receivedTokenIdPadded
        console.log('- - token id  - ->  ' + id)
        console.log(' ')
        // Grant reception approval.  -Unix Time 21 10:00:01 2030 UTC <- 1895220001
        idcs[2].methods
            .approveSender(energyToken.address, idcs[0].options.address, '1895220001', '17000000000000000000', id)
            .send({ from: accounts[7], gas: 7000000 })

        // Create forwards.
        let abiCreateForwardsCall = energyTokenWeb3.methods
            .createForwards(1737540001, 2, distributor.address)
            .encodeABI()

        //console.log('abiCreateForwardsCall -> ' + abiCreateForwardsCall)

        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwardsCall)
            .send({ from: accounts[5], gas: 7000000 })

        // Perform actual mint operation via execute() of IDC 0.
        let abiMintCall = energyTokenWeb3.methods
            .mint(id, [idcs[2].options.address], ['17000000000000000000'])
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiMintCall)
            .send({ from: accounts[5], gas: 7000000 })
    ...
    it.skip('can create generation-based forwards.', async function () {
        let balancePeriod = '1895220001'
        let abiCreateTokensCall = energyTokenWeb3.methods
            .createForwards(balancePeriod, 1, idcs[1].options.address)
            .encodeABI()
        let callResult = await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateTokensCall)
            .send({ from: accounts[5], gas: 7000000 })
    ...
    it.skip('can transfer tokens.', async function () {
        ...
        let abiApproveSenderCall = idcs[1].methods
            .approveSender(energyToken.address, idcs[2].options.address, '1895220001', '5000000000000000000', id)
            .encodeABI()
        await idcs[1].methods
            .execute(0, idcs[1].options.address, 0, abiApproveSenderCall)
            .send({ from: accounts[6], gas: 7000000 })
        ...
        let jsonAcceptedDistributor =
            '{ "t": "t", "expiryDate": "1895220001", "startDate": "1", "address": "' +
            idcs[1].options.address.slice(2).toLowerCase() +
            '" }'
        let dataAcceptedDistributor = web3.utils.toHex(jsonAcceptedDistributor)
        await addClaim(idcs[1], 10120, balanceAuthority.options.address, dataAcceptedDistributor, '', account8Sk)

        // Send 5 tokens.
        let abiTransfer = energyTokenWeb3.methods
            .safeTransferFrom(idcs[2].options.address, idcs[1].options.address, id, '5000000000000000000', '0x00')
            .encodeABI()
        await idcs[2].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiTransfer)
            .send({ from: accounts[7], gas: 7000000 })
        ...
        // Upper edge case.
        let abiTransferUpper = energyTokenWeb3.methods
            .safeTransferFrom(idcs[2].options.address, idcs[1].options.address, id, '2000000000000000001', '0x00')
            .encodeABI()
        await truffleAssert.reverts(
            idcs[2].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiTransferUpper)
                .send({ from: accounts[7], gas: 7000000 })
        )

        // Lower edge case.
        let abiTransferLower = energyTokenWeb3.methods
            .safeTransferFrom(idcs[2].options.address, idcs[1].options.address, id, '1999999999999999999', '0x00')
            .encodeABI()
        await truffleAssert.reverts(
            idcs[2].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiTransferLower)
                .send({ from: accounts[7], gas: 7000000 })
        )
        ...
        let abiTransferSelf1 = energyTokenWeb3.methods
            .safeTransferFrom(idcs[2].options.address, idcs[2].options.address, id, '5000000000000000000', '0x00')
            .encodeABI()
        await truffleAssert.reverts(
            idcs[2].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiTransferSelf1)
                .send({ from: accounts[7], gas: 7000000 })
        )

        // Transferring tokens to a receiver without the necessary claims needs to fail.
        let abiApproveSenderCallDistributor = distributorWeb3.methods
            .approveSender(energyToken.address, idcs[2].options.address, '1895220001', '1000000000000000000', id)
            .encodeABI()
        await distributorWeb3.methods
            .execute(0, distributorWeb3.options.address, 0, abiApproveSenderCallDistributor)
            .send({ from: accounts[0], gas: 7000000 })
        let abiTransferToDistributor = energyTokenWeb3.methods
            .safeTransferFrom(
                idcs[2].options.address,
                distributorWeb3.options.address,
                id,
                '1000000000000000000',
                '0x00'
            )
        ...
        await truffleAssert.reverts(
            idcs[2].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiTransferToDistributor)
                .send({ from: accounts[7], gas: 7000000 })
        )
    })
    ...
    it.skip('can perform batch transfers.', async function () {
        ...
        let abiApproveSenderCall11 = idcs[1].methods
            .approveSender(energyToken.address, idcs[0].options.address, '1895220001', '17000000000000000000', id1)
            .encodeABI()
        await idcs[1].methods
            .execute(0, idcs[1].options.address, 0, abiApproveSenderCall11)
            .send({ from: accounts[6], gas: 7000000 })
        let abiApproveSenderCall12 = idcs[1].methods
            .approveSender(energyToken.address, idcs[0].options.address, '1895220001', '17000000000000000000', id2)
            .encodeABI()
        await idcs[1].methods
            .execute(0, idcs[1].options.address, 0, abiApproveSenderCall12)
            .send({ from: accounts[6], gas: 7000000 })

        // Forwards creation.
        let abiCreateForwardsCall1 = energyTokenWeb3.methods
            .createForwards(1737540901, 2, distributor.address)
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwardsCall1)
            .send({ from: accounts[5], gas: 7000000 })
        let abiCreateForwardsCall2 = energyTokenWeb3.methods
            .createForwards(1737549901, 2, distributor.address)
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwardsCall2)
            .send({ from: accounts[5], gas: 7000000 })

        // Minting.
        let abiMintCall1 = energyTokenWeb3.methods
            .mint(id1, [idcs[1].options.address], ['17000000000000000000'])
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiMintCall1)
            .send({ from: accounts[5], gas: 7000000 })
        let abiMintCall2 = energyTokenWeb3.methods
            .mint(id2, [idcs[1].options.address], ['17000000000000000000'])
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiMintCall2)
            .send({ from: accounts[5], gas: 7000000 })

        // Reception approval is required for forwards.
        let abiApproveSenderCall21 = idcs[2].methods
            .approveSender(energyToken.address, idcs[1].options.address, '1895220001', '1000000000000000000', id1)
            .encodeABI()
        await idcs[2].methods
            .execute(0, idcs[2].options.address, 0, abiApproveSenderCall21)
            .send({ from: accounts[7], gas: 7000000 })
        let abiApproveSenderCall22 = idcs[2].methods
            .approveSender(energyToken.address, idcs[1].options.address, '1895220001', '3000000000000000000', id2)
            .encodeABI()
        await idcs[2].methods
            .execute(0, idcs[2].options.address, 0, abiApproveSenderCall22)
            .send({ from: accounts[7], gas: 7000000 })

        // Transfer.
        let abiBatchTransfer = energyTokenWeb3.methods
            .safeBatchTransferFrom(
                idcs[1].options.address,
                idcs[2].options.address,
                [id1, id2],
            ...
            .encodeABI()
        await idcs[1].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiBatchTransfer)
            .send({ from: accounts[6], gas: 9000000 })
    ...
    it.skip('rejects too big generation energy documentations', async function () {
        let balancePeriod = 1737524701

        // Forwards must be created first so distributor is set.
        let abiCreateForwards = energyTokenWeb3.methods
            .createForwards(balancePeriod, 0, distributorWeb3.options.address)
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwards)
            .send({ from: accounts[5], gas: 7000000 })

        // Zero must work.
        let abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[0].options.address, 0, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
            .send({ from: accounts[8], gas: 7000000 })

        // Must work right up to the maximum.
        abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[0].options.address, '75000000000000000000000', balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
            .send({ from: accounts[8], gas: 7000000 })
        ...
        abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[0].options.address, '75000000000000000000001', balancePeriod)
            .encodeABI()
        await truffleAssert.reverts(
            meteringAuthority.methods
                .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
                .send({ from: accounts[8], gas: 7000000 })
        )
    })

    it.skip('rejects too big consumption energy documentations', async function () {
        let balancePeriod = 1737524701

        // Zero must work.
        let abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[2].options.address, 0, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
            .send({ from: accounts[8], gas: 7000000 })
        ...
        // Must work right up to the maximum.
        abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[2].options.address, '37500000000000000000000', balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
            .send({ from: accounts[8], gas: 7000000 })
        ...
        // Must not work for 1 above the maximum.
        abiUpdateEnergyDoc = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[2].options.address, '37500000000000000000001', balancePeriod)
            .encodeABI()
        await truffleAssert.reverts(
            meteringAuthority.methods
                .execute(0, energyTokenWeb3.options.address, 0, abiUpdateEnergyDoc)
    ...
    it.skip('distributes tokens correctly.', async function () {
        ...
        let jsonAcceptedDistributor =
            '{ "t": "t", "expiryDate": "1895220001", "startDate": "1", "address": "' +
            distributorWeb3.options.address.slice(2).toLowerCase() +
            '" }'
        let dataAcceptedDistributor = web3.utils.toHex(jsonAcceptedDistributor)
        await addClaim(
            distributorWeb3,
            10120,
            balanceAuthority.options.address,
            dataAcceptedDistributor,
            '',
            account8Sk
        )
        ...
        for (let forwardKind = 0; forwardKind <= 2; forwardKind++) {
            // Get certificate ID.
            let receivedCertificateId = await energyToken.getTokenId(
                3,
                balancePeriod + 9000 * forwardKind,
                idcs[0].options.address
            )
        ...
        for (let forwardKind = 0; forwardKind <= 2; forwardKind++) {
            // Get forward ID.
            let receivedForwardId = await energyToken.getTokenId(
                forwardKind,
                balancePeriod + 9000 * forwardKind,
                idcs[0].options.address
            )
        ...
        for (let forwardKind = 0; forwardKind <= 2; forwardKind++) {
            let abiCreateForwards = energyTokenWeb3.methods
                .createForwards(balancePeriod + 9000 * forwardKind, forwardKind, distributorWeb3.options.address)
                .encodeABI()
            await idcs[0].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwards)
                .send({ from: accounts[5], gas: 7000000 })

            if (forwardKind == 1) continue // Generation-based forwards cannot be minted.
            ...
            idcs[1].methods
                .approveSender(
                    ...
                    forwardIds[forwardKind]
                )
                .send({ from: accounts[6], gas: 7000000 })

            // Perform actual mint operation via execute() of IDC 0.
            let abiMintCall = energyTokenWeb3.methods
                .mint(forwardIds[forwardKind], [idcs[1].options.address], ['17000000000000000000'])
                .encodeABI()
            await idcs[0].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiMintCall)
                .send({ from: accounts[5], gas: 7000000 })
        ...
        idcs[1].methods
            ...
            .send({ from: accounts[6], gas: 7000000 })
        let abiTransferGenerationBasedForwards = energyTokenWeb3.methods
            .safeTransferFrom(
                idcs[0].options.address,
                idcs[1].options.address,
                forwardIds[1],
            ...
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiTransferGenerationBasedForwards)
            .send({ from: accounts[5], gas: 7000000 })

        let distributeCall = async function (forwardKind) {
            await distributorWeb3.methods
                .distribute(idcs[1].options.address, forwardIds[forwardKind])
                .send({ from: accounts[0], gas: 7000000 })
        }
    ...
    */
        ...
        for (let forwardKind = 0; forwardKind <= 2; forwardKind++) {
            ...
            let abiAddGenerationCall1 = energyTokenWeb3.methods
                .addMeasuredEnergyGeneration(
                    idcs[0].options.address,
                ...
                .encodeABI()
            await meteringAuthority.methods
                .execute(0, energyTokenWeb3.options.address, 0, abiAddGenerationCall1)
                .send({ from: accounts[8], gas: 7000000 })

            // Add energy consumption.
            let abiAddConsumptionCall1 = energyTokenWeb3.methods
                .addMeasuredEnergyConsumption(
                    idcs[1].options.address,
                    '8000000000000000000',
                    balancePeriod + 9000 * forwardKind
                )
                .encodeABI()
            await meteringAuthority.methods
                .execute(0, energyTokenWeb3.options.address, 0, abiAddConsumptionCall1)
                .send({ from: accounts[8], gas: 7000000 })
        }
        ...
        let abiTransferCertsBack = energyTokenWeb3.methods
            .safeTransferFrom(
                idcs[1].options.address,
                distributorWeb3.options.address,
                certificateIds[0],
            ...
            .encodeABI()
        await idcs[1].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiTransferCertsBack)
            .send({ from: accounts[6], gas: 7000000 })
        ...
        // Perform actual mint operation via execute() of IDC 0.
        let abiMintCall = energyTokenWeb3.methods
            .mint(forwardIds[0], [idcs[2].options.address], ['83000000000000000000'])
            .encodeABI()
        await idcs[0].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiMintCall)
            .send({ from: accounts[5], gas: 7000000 })
        ...
        // Send certificates back for another test.
        abiTransferCertsBack = energyTokenWeb3.methods
            .safeTransferFrom(
                idcs[1].options.address,
                distributorWeb3.options.address,
                certificateIds[0],
    ...
    */
    ...
    it.skip('distributes surplus certificates correctly.', async function () {
        ...
        let surplusWithdrawalCall = async function (forwardKind) {
            await distributorWeb3.methods
                .withdrawSurplusCertificates(forwardIds[forwardKind])
                .send({ from: accounts[0], gas: 7000000 })
        }
    ...
    it.skip('keeps track of energy data.', async function () {
        let json = '{ "q": "ab", "expiryDate": "1895220001", "startDate": "1", "realWorldPlantId": "bestPlantId" }'
        let data = web3.utils.toHex(json)
        const jsonExistenceGeneration =
            '{ "type": "generation", "expiryDate": "1895220001", "startDate": "1", "realWorldPlantId": "bestPlantId" }'
        ...
        let abiAddConsumptionCall1 = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[1].options.address, 500, balancePeriod)
            .encodeABI()
        ...
        let abiAddConsumptionCall2 = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[1].options.address, 335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddConsumptionCall2)
            .send({ from: accounts[8], gas: 7000000 })

        // It needs to be possible to update the value again.
        let abiAddConsumptionCall3 = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[1].options.address, 1335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddConsumptionCall3)
            .send({ from: accounts[8], gas: 7000000 })

        // And again.
        let abiAddConsumptionCall4 = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[1].options.address, 2335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddConsumptionCall4)
            .send({ from: accounts[8], gas: 7000000 })

        let energyConsumptionDocumentation = await energyTokenWeb3.methods
            .energyDocumentations(idcs[1].options.address, balancePeriod)
            .call()
        ...
        let abiCreateForwardsCall = energyTokenWeb3.methods
            .createForwards(balancePeriod, 2, distributor.address)
            .encodeABI()
        await idcs[2].methods
            .execute(0, energyTokenWeb3.options.address, 0, abiCreateForwardsCall)
            .send({ from: accounts[7], gas: 7000000 })
        ...
        // The call must revert if it comes from something that's not a metering authority's IDC (even if it's by the metering authority directly).
        let abiAddGenerationCall1 = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[2].options.address, 500, balancePeriod)
            .encodeABI()
        await truffleAssert.reverts(
            idcs[0].methods
                .execute(0, energyTokenWeb3.options.address, 0, abiAddGenerationCall1)
        ...
        // It needs to be possible to update the value.
        let abiAddGenerationCall2 = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[2].options.address, 335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddGenerationCall2)
            .send({ from: accounts[8], gas: 7000000 })

        // It needs to be possible to update the value again.
        let abiAddGenerationCall3 = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[2].options.address, 1335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddGenerationCall3)
            .send({ from: accounts[8], gas: 7000000 })

        // And again.
        let abiAddGenerationCall4 = energyTokenWeb3.methods
            .addMeasuredEnergyGeneration(idcs[2].options.address, 2335, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddGenerationCall4)
            .send({ from: accounts[8], gas: 7000000 })
        ...
        let abiAddConsumptionCall6 = energyTokenWeb3.methods
            .addMeasuredEnergyConsumption(idcs[0].options.address, 10000, balancePeriod)
            .encodeABI()
        await meteringAuthority.methods
            .execute(0, energyTokenWeb3.options.address, 0, abiAddConsumptionCall6)
            .send({ from: accounts[8], gas: 7000000 })

        assert.equal((await energyToken.energyDocumentations(idcs[0].options.address, balancePeriod)).value, 10000)

        let energyConsumptionDocumentation2 = await energyTokenWeb3.methods
            .energyDocumentations(idcs[0].options.address, balancePeriod)
            .call()
        assert.equal(energyConsumptionDocumentation2.documentingMeteringAuthority, meteringAuthority.options.address)

        let energyGenerationDocumentation = await energyTokenWeb3.methods
            .energyDocumentations(idcs[2].options.address, balancePeriod)
            .call()
